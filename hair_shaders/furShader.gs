...
layout(line_strip, max_vertices = 39) out; // Unfortunately this needs to be hardcoded.. (3*(noOfHairSegments+1))Do not forget to change this
                                           ...
                                           //set output layout to line strips with max 39 vertices
uniform mat4 model;
uniform mat4 view;
...
//create hair according to the trangle and number of hair segments.
void generateHairStrands(int index){
    vec3 lastPos = (gl_in[index].gl_Position).xyz;  // the start pos of the hair
...
//代码中的getInterpolatedPosition函数根据纹理获取每个顶点的位置，然后使用曲面参数teTessCoords计算当前位置。
vec3 getInterpolatedPosition(int index, int hairIndex){
...
//getPositionFromTexture函数则根据顶点索引和发丝索引从纹理中获取顶点的位置。
vec3 getPositionFromTexture(float vertexIndex, float hairIndex) {
